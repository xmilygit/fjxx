...
router.get('/fenliu', function(req, res, next) {
    if (req.session.openid) {
        res.render('graduate/wechat/fenliu', {
            title: '自愿分流意向',
            layout: 'f7layoutsbase'
        })
    } else {
...
router.get('/dispresult', function(req, res, next) {
    ...
    if (openid) {
        ...
        mongoose.model('Account').findOne({ wxopenid: openid }, { infoid: 1 }, function(err, doc) {
            ...
            query.exec(function(err, stuinfo) {
                ...
                //console.log(stuinfo)
                var msg = stuinfo.材料.replace(/\n/gi, "<br>")
                res.render('graduate/wechat/dispresult', {
                    title: '材料提交参考',
                    layout: 'f7layoutsbase',
                    msg: msg
                })
...
router.post('/Savefenliu', function(req, res, next) {
    res.setHeader("Access-Control-Allow-Origin", "*");
    var openid = req.session.openid; //"o_BZpuDFj3Gi-psvtFFDRgl9id-0"; //"o_BZpuDFj3Gi-psvtFFDRgl9id-0";//
    ...
    mongoose.model('Account').findOne({ wxopenid: openid }, { infoid: 1 }, function(err, doc) {
        ...
        mongoose.model('StudentInfo').findByIdAndUpdate(doc.infoid, { 'school1': stdata.school1, 'school2': stdata.school2, 'school3': stdata.school3 }, { runValidators: true, upsert: true }, function(err, doc2) {
            if (err) {
                res.json({ 'error': true, 'message': String(err).replace('ValidationError: ', '') });
                return;
            }
...
router.post('/GetInfoByOpenId', function(req, res, next) {
    //console.log(req.session.openid)
    res.setHeader("Access-Control-Allow-Origin", "*");
    var openid = req.body.openid; //"o_BZpuDFj3Gi-psvtFFDRgl9id-0"; //req.session.openid;//
    mongoose.model('Account').findOne({ wxopenid: openid }, { infoid: 1 }, function(err, doc) {
        if (err) {
            if (err) {
                res.json({ 'error': true, 'message': err });
